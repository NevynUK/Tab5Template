 ...
 * Description : WiFi Scanner example application for the M5Stack Tab5 firmware.
 *               Initialises all on-board hardware (display, touch, SD card,
 *               RTC, IMU, power monitor, charge manager, RS-485 and camera),
 *               renders a status splash, then enters the WiFi scanning demonstration.
 * Author      : Mark Stevens
...
const char *LOG_TAG = "WiFiScanner";
...
void Setup(void)
{
    ...
    display.drawString("WiFi Scanner", centreX, lineY);
    lineY += lineHeight;
